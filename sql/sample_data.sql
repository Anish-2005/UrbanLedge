...
-- Generate assessments for sample properties using sp_generate_assessment or manual inserts
...
-- After payment, receipts are auto-created by trigger

-- Example queries
-- 1. Outstanding assessments
SELECT * FROM assessment WHERE total_due > 0;

-- 2. Revenue by ward
SELECT w.name, SUM(paid_amount) AS revenue
FROM payment pm
JOIN assessment a ON pm.assess_id = a.assess_id
